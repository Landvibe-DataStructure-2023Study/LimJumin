...
/*
 특정 폴더의 용량을 계산하는 프로그램
 폴더의 용량 : 자신의 크기와 하부 폴더들의 크기의 합 (크기 ≠ 값)
 */
...
public:
    ...
    /*
     용량 계산 함수
     용량 : 자신의 크기와 폴더의 하부 폴더들의 크기의 합 (자신 포함에 주의)
     */
    void countFiles(Node* node) {
        sizeOfFiles += weight[node->value]; // 노드의 번호를 배열의 인덱스로 삼아서 배열의 값인 크기에 접근
        
        for(Node* child : node->childList) { // 재귀로 자식 순회
            countFiles(child);
        }
    }
};
...
int main() {
    /*
     ...
     첫째 줄에는 전체 폴더와 파일의 개수 N(2≤N≤1000)과 파일의 개수를 계산할 폴더의 수 M(1≤M≤1000)이 공백으로 구분되어 주어진다.
       ...
       - N개 줄에 걸쳐, 각 폴더의 번호 x와 크기 w(1≤w≤1,000)가 공백으로 구분되어 주어진다.
    ...
    // 노드 번호를 배열의 인덱스로 삼아서 크기를 배열의 값으로 저장
    for(int i=0; i<N; i++) {
        cin >> x >> w;
        weight[x] = w;
    }
