...
/*
 ...
 (insert, delete, parent, child, maxChild)
 */
...
public:
    ...
    Tree() { // 생성자
        root = new Node(nullptr, 1); // 부모 없고 1값을 가지는 root 노드 생성(문제에서 항상 루트 노드 1이 있다고 나와있어서)
    ...
    /*
     ...
     2. x값을 가진 노드가 없으면 -1을 출력한다.
     3. y가 이미 트리에 존재할 경우 -1을 출력한다.
     */
    void insertNode(int x, int y) { // 삽입 연산(트리에 노드 추가)
        ...
        if (parentNode == nullptr) { // x값을 가진 노드가 없을 경우
            cout << "-1\n"; // 문제조건따라 에러 처리
        } else if (findNode(y) != nullptr) { //y값을 가진 노드가 이미 트리에 존재하는 경우
            cout << "-1\n";
        } else { // 올바르게 추가하는 경우
    ...
    /*
     노드 x(1≤x≤10000)의 자식 노드를 삽입된 순서대로 공백으로 출력한다.
     delete 명령어로 인해 x에 연결된 자식들도 연결 당시 삽입된 것으로 간주한다.
     만약 x의 자식노드가 존재하지 않거나 x가 트리에 존재하지 않을 경우 -1 출력한다.
     */
    void child(int x) {
        Node* curNode = findNode(x);
    ...
    /*
     노드 x의 자식 노드에 저장된 정수 중 최대값을 출력한다.
     만약 x의 자식 노드가 존재하지 않거나 x가 트리에 존재하지 않을 경우 -1 출력한다.
     */
    void maxChild(int x) {
        Node* curNode = findNode(x);
        ...
        if (curNode == nullptr || curNode->childList.empty()) {
            ...
            int maxChild = -1;
            for(Node* child : curNode->childList) {
                if (child->value > maxChild) {
                    maxChild = child->value;
                }
            }
            /*
             또는 아래와 같이 사용 (편한대로 사용)
             for(int i=0; i<curNode->childList.size(); i++) {
                 if (curNode->childList[i] > maxChild) {
                     maxChild = curNode->childList[i]->value;
                 }
             }
             */
            cout << maxChild << "\n";
...
/*
 3. 메인 함수 구현
 첫째 줄에는 함수 입력 개수 T(1≤N≤10000)이 주어진다.
