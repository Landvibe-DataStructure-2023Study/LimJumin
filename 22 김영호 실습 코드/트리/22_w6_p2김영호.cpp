...
/*
 해당 노드의 모든 조상들을 공백으로 구분하여 출력하는 프로그램
 */
...
public:
    ...
    Tree() { // 생성자
        root = new Node(nullptr, 1); // 부모 없고 1값을 가지는 root 노드 생성(문제에서 루트 노드는 1이라고 나와있어서)
    ...
    /*
     삽입 연산
     y값을 가진 노드를 x값을 가진 노드의 자식으로 삽입한다.
     */
...
/*
 3. 메인 함수 구현
 첫째 줄에는 트리의 노드 개수 N(1≤N≤1000)과 질문 수 M(1≤M≤N)이 주어짐
 둘째 줄부터 'N-1'개의 줄에 걸쳐 부모 노드와 삽입될 자식 노드가 순서대로 주어진다.
 이후 'M'개의 줄에 걸쳐 조상들을 출력할 노드의 번호가 주어진다.
 */
int main() {
