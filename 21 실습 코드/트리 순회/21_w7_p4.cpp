...
/*
 트리를 후위 순회하며 노드의 저장된 값과 깊이를 출력한 결과가 주어질 때,
 전위 순회한 결과를 출력하는 프로그램.
 */
...
public:
    ...
    void preOrderTraversal(Node* node) {
        ...
        // 메인 함수에서 생성된 트리가 좌우반전된 결과이기 때문에 끝에서부터 접근하면서 출력해준다.
        for(int i = node->childList.size()-1; i>=0; i--) {
            preOrderTraversal(node->childList[i]);
        }
    }
};
...
int main() {
    /*
     ...
     첫째 줄에는 테스트 케이스의 개수 T(1≤T≤100)가 주어진다. 그 후 T회에 걸쳐 각 트리의 전위순회한 결과가 아래와 같이 주어진다.
       ...
       - 각 노드의 '저장된 값' X(1≤X≤100000)가 후위 순회 순서에 맞춰 공백으로 구분하여 주어진다.
       - 각 노드의 '깊이'가 후위 순회 순서에 맞춰 공백으로 구분하여 주어진다.
     */
    int T, N;
    cin >> T;
    while(T--) {
        ...
        // 2. 트리 생성
        // 배열의 끝에서부터 접근(후위 순회에서는 마지막에 루트 등장->루트부터 접근하기 위해 배열 끝에서부터 순회)
        // 이렇게 하면 생성된 트리는 원래 트리의 좌우반전된 결과로 나오는데, 그러면 이후 출력 함수(전위 순회 함수)에서도 childList를 역순회(끝에서부터 접근)하면서 출력해주면 된다.
        for(int i=N-1; i>0; i--) {
