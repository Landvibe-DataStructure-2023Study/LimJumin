...
class Queue { // 필요한 함수만 구현, 큐의 사이즈가 0인 경우 고려할 필요 없음, 큐 사이즈 넘어갈 일 안일어남
public:
    ...
    while(numOfTest--) {
        ...
        for(int i=0; i<numOfCard; i++) { // 카드 개수인 n 만큼 게임 반복 실행
            ...
            if (cardOfP1 > cardOfP2) { // 플레이어 1가 win한 경우
                surplusHp2 = cardOfP1 - cardOfP2 - 1; // 플레이어 2(패자)에게 더해줄 잔여체력 할당
                surplusHp1 = 0; // 플레이어 1에게는 더해줄 잔여체력 없음
                pointOfP1++; // 플레이어 1 승점 추가
            } else if (cardOfP2 > cardOfP1) { // 플레이어 2가 win한 경우
                surplusHp1 = cardOfP2 - cardOfP1 - 1; // 플레이어 1(패자)에게 더해줄 잔여체력 할당
