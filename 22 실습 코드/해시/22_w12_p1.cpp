...
public:
    ...
    /*
     erase K
     키 K에 해당하는 원소를 해시테이블에서 삭제 후 출력, 존재하지 않으면 None 출력
     */
    void erase(int key) {
        ...
        while(hashTable[idx].valid != NOITEM && probing <= capacity) {
            ...
            if(hashTable[idx].valid == ISITEM && hashTable[idx].key == key) {
                ...
                hashTable[idx].valid = AVAILABLE; // 키를 찾으면 해시테이블에서 출력과 삭제 후 리턴
                return;
            }
