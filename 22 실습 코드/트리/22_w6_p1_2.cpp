...
/*
 같은 부모 노드를 가지는 자식 노드들을 출력하는 프로그램
 (insert, delete, print)
 */
...
public:
    ...
    /*
     ...
     3. y값이 중복으로 들어오는 경우는 테스트 케이스에 존재하지 않는다.
     */
    ...
    /*
     ...
     3. 루트 노드를 삭제하려고 하는 경우는 테스트 케이스에 존재하지 않는다.
     */
    ...
    /*
     출력 연산 (print x)
     1. x값을 가진 노드와 같은 부모를 가지는 자식 노드와 x 노드를 출력한다.
     2. 다른 자식 노드가 없거나 부모 노드가 없을 경우 x 노드만 출력한다.
     3. 해당 x 노드가 없을 시 0 출력한다.
     */
    void printNode(int x) {
        ...
        if (curNode == nullptr) {
            cout << "0\n";
        } else if (curNode == root || curNode->parent->childList.empty()) {
            cout << x << "\n";
        } else {
            Node *parentNode = curNode->parent;
            for (int i=0; i<parentNode->childList.size(); i++) { // 자식 노드의 수만큼 반복
                cout << parentNode->childList[i]->value << " "; // 자식 노드의 값 출력
            }
            cout << "\n";
        }
    }
