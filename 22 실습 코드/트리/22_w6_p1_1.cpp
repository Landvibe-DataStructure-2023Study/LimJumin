...
/*
 특정 노드의 자식을 출력하는 프로그램 (21년도 6주차 1번 문제와 동일)
 (insert, delete, print)
 */
...
public:
    ...
    Node *findNode(int v) { // 인자값에 해당하는 노드의 위치 반환
    ...
    /*
     ...
     2. x값을 가진 노드가 없으면 삽입 취소한다.
     3. y는 중복으로 들어오는 경우가 없다. (문제에 번호가 같은 노드는 2번 이상 생성되는 경우 없다고 나옴.)
     */
    ...
    /*
     ...
     2. x값을 가진 노드가 없을 경우 무시한다.
     3. 루트 노드를 삭제하려고 하는 경우 테스트 케이스에 없다고 나와있어서 고려 안해줘도 됨
     */
    ...
    /*
     출력 연산 (print x)
     1. x값을 가진 노드의 자식 노드들을 삽입 순으로 출력한다.
     2. 만약 자식 노드가 없거나 x값을 가진 노드가 없으면 0 출력한다.
     */
    void printNode(int x) {
        ...
        if (curNode == nullptr || curNode->childList.empty()) {
            ...
            for (int i=0; i<curNode->childList.size(); i++) { // 자식 노드의 수만큼 반복
                cout << curNode->childList[i]->value << " "; // 자식 노드의 값 출력
            }
            /*
             또는 다음과 같이 사용 (간단한 버전)
